...
def train_meta(args):
    ...
    F1_test = -1.0
    best_step, protect_step = -1.0, 10
    #best_step, protect_step = -1.0, 30 if args.train_mode != "type" else 30
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--types_list_path",
        type=str,
        help="the path of entities types list",
        default="data/entity_types_list.json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--distance_mode", type=str, help="embedding distance mode", default="cat"
    )
    ...
    if "Domain" in args.dataset:
        args.types_list_path = "data/entity_types_list_domain.json"
    ...
    if args.test_only:
        ...
        if not os.path.exists("{}/{}".format(top_dir, args.result_dir)):
            ...
            pass
        # in the following line, we update the result dir
    ...
    args.entity_types = EntityTypes(
        args.types_path, args.negative_types_number, args.negative_mode, args.types_list_path
    )

    # we will insert text file here

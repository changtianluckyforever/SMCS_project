...
class Learner(nn.Module):
        ...
        if model_dir != "":
            ...
            if args.add_trained_span:
                ...
                self.model = BertForTokenClassification_.from_pretrained(
                    bert_model, num_labels=num_labels, output_hidden_states=True, mirror='https://mirrors.tuna.tsinghua.edu.cn/hugging-face-models',
                ).to(args.device)
                self.model.load_state_dict(torch.load(add_trained_span_path, map_location="cuda"))
                print('we already loaded the trained span')
            else:
    ...
    def load_model(self, mode: str = "all"):
        ...
        self.model = BertForTokenClassification_.from_pretrained(
            self.bert_model, num_labels=len(self.label_list), output_hidden_states=True, mirror='https://mirrors.tuna.tsinghua.edu.cn/hugging-face-models',
        )
        self.model.set_config(
